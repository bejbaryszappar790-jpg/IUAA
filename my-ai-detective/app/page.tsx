...
import { ShieldCheck, User, Search, Brain, Activity, FileUp, AlertCircle } from "lucide-react";

// Пути к компонентам (убедись, что папка src/ui находится в корне или на уровень выше)
...
export default function Home() {
  ...
  const handleStartAnalysis = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file) return alert("Бейбарыс, файлды жүктеуді ұмыттың ба? (Забыл загрузить файл)");
    ...
    data.append("cert_type", "IELTS"); // Упростили тип для соответствия scorer.py
    data.append("file", file);

    try {
      // ИСПОЛЬЗУЕМ LOCALHOST ВМЕСТО IP ХАРУНА
      const res = await axios.post("http://localhost:8000/analyze", data);
      setResult(res.data);
    } catch (err: any) {
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col items-center mb-16 text-center">
          <motion.div 
            ...
            className="p-3 bg-blue-600/10 border border-blue-500/20 rounded-2xl mb-6"
          >
          ...
          <p className="text-slate-500 uppercase tracking-[0.3em] text-[10px] font-bold">AI Verification Protocol v1.0 | Local Mode</p>
        </header>
            ...
            <Card title="Параметры объекта" icon={Search}>
              <form onSubmit={handleStartAnalysis} className="space-y-8">
                <Input 
                  label="ФИО Кандидата" 
                  icon={User} 
                  placeholder="Напр: Арманулы Бейбарыс" 
                  value={name} 
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1 flex items-center gap-2">
                    <FileUp size={12} className="text-blue-500" /> Цифровой оттиск (PDF/JPG)
                  </label>
                ...
                {error && (
                  <div className="flex items-center gap-2 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl text-red-400 text-xs">
                    <AlertCircle size={16} />
                    <span>{error}. Проверь: запущен ли `python src/api/main.py`?</span>
                  </div>
                )}
              ...
              {result ? (
                <Card title="Результат сканирования" icon={Brain}>
                  <div className="space-y-8">
                    {/* Отчет AI */}
                    <div className="bg-blue-600/5 border-l-2 border-blue-600 p-6 rounded-r-2xl italic text-sm leading-relaxed text-blue-100/80 shadow-inner whitespace-pre-line">
                      {result.ai_report}
                    </div>

                    {/* Сетка баллов */}
                    <div className="grid grid-cols-2 gap-4">
                      {Object.entries(result.ai_scores || {}).map(([key, val]: any) => (
                        <div key={key} className="bg-slate-900/60 border border-slate-800 p-5 rounded-3xl text-center hover:border-blue-500/30 transition-all">
                          ...
                          <p className="text-4xl font-black text-white">{val}<span className="text-xs text-slate-600 ml-1">/10</span></p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="h-[480px] border-2 border-dashed border-slate-900 rounded-[2.5rem] flex flex-col items-center justify-center p-12 text-slate-800 bg-slate-900/5">
                  ...
                  <p className="mt-4 text-xs text-slate-700 italic">Ожидаю ввод данных объекта...</p>
                </div>
              )}
            </AnimatePresence>
          </motion.div>
